...
def split(df, validation_ratio):
    ...
    train, test = h2o.H2OFrame(X).split_frame(
        ratios=[validation_ratio], seed=1)
...
def auto_ML(df, n_models, validation_ratio=0.5):
    """
    Initialize h2o, a new Auto ML object and already train it applying
    verification following the validation ratio.
    """
...
def gen_predictions(df, model, validation_ratio=0.5, return_len=True):
    """
    Receives DataFrame and model, it split the frame
    and returns predictions and len of splits.
    """
    X, y, train, test = split(df, validation_ratio)
...
def gen_models_predictions(df, models, validation_ratio=0.5):
    """
    Receives DataFrame, dict fo models and returns a DataFrame with N + 1
    columns.
    N is the number of models on dict (including Stacked Ensemble) plus the
    true value as first column (REDSHIFT_SPEC).
    """
    X, y, train, test = split(df, validation_ratio)
    ...
    for model_name in models.keys():
        preds = gen_predictions(
            df, models[model_name], validation_ratio=validation_ratio,
            return_len=False
        )
...
def print_gaussian_kde(pdf, preds, real_value, best_pred, title, points=100):
    """
    Receives the Probability Density Function generated
    by gaussian KDE method and prints it
    ...
    axs.hist(preds, density=True, histtype="step",
             label="Models prediction histogram")
    axs.legend(loc="best")
    axs.set_title(title)
